  ...
  return (
    <div className="space-y-4" dir="rtl">
      <div className="flex items-center gap-2">
        ...
        <h2 className="text-lg font-bold sm:text-xl">متابعة الواجبات</h2>
      </div>
  ...
  return (
    <Card className="overflow-hidden border-none shadow-md bg-white dark:bg-slate-900">
      <div className="bg-primary/5 px-4 py-3 border-b border-primary/10">
        <h3 className="flex min-w-0 items-center gap-2 text-base font-bold text-primary sm:text-lg">
          <GraduationCap className="h-5 w-5" />
          <span className="truncate">{data.childName}</span>
        </h3>
      </div>
      <CardContent className="p-4 space-y-4">
        ...
        {data.lastGrade && (
          <div className="space-y-2">
            ...
            <div className="flex flex-col gap-3 rounded-xl border border-emerald-100 bg-emerald-50 p-3 dark:border-emerald-900/50 dark:bg-emerald-950/20 min-[420px]:flex-row min-[420px]:items-center min-[420px]:justify-between">
              <div className="flex min-w-0 items-center gap-3">
                <CheckCircle2 className="h-4 w-4 text-emerald-600" />
                <p className="min-w-0 truncate text-sm font-bold text-emerald-900 dark:text-emerald-200">{data.lastGrade.assignmentTitle}</p>
              </div>
