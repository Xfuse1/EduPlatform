...
export function StatsCard({
  ...
  return (
    <Card className={cn("overflow-hidden border border-[rgba(255,255,255,0.08)] bg-[rgba(255,255,255,0.05)] backdrop-blur-[16px] rounded-[16px] text-white", toneStyles[tone])}>
      <CardContent className="p-6">
        <div className="flex items-start justify-between gap-4">
          <div className="min-w-0 flex-1">
            <p className="text-start text-sm font-semibold text-[#94A3B8]">{title}</p>
            <p className={cn("mt-3 text-start text-3xl font-extrabold tracking-tight", toneColors[tone])}>{value}</p>
            {hint ? (
              <div className="mt-4 inline-flex min-h-11 items-center gap-2 rounded-full bg-white/10 px-3 py-2 text-xs font-bold text-white/80">
                <TrendIcon className={cn("h-4 w-4", toneColors[tone])} />
                <span>{hint}</span>
              </div>
            ) : null}
          </div>
          <div className={cn("flex h-14 w-14 shrink-0 items-center justify-center rounded-[20px] bg-white/10 shadow-[inset_0_1px_0_rgba(255,255,255,0.1)]", toneColors[tone])}>
